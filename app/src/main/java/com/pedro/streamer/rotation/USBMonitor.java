...
public final class USBMonitor {
  ...
  public USBMonitor(final Context context, final USBMonitor.OnDeviceConnectListener listener) {
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
   ...
   */
  public USBMonitor.UsbControlBlock openDevice(final UsbDevice device) throws SecurityException {
    if (hasPermission(device)) {
      USBMonitor.UsbControlBlock result = mCtrlBlocks.get(device);
      if (result == null) {
        result = new USBMonitor.UsbControlBlock(USBMonitor.this, device);    // この中でopenDeviceする
...
//		if (DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
   ...
   * @return
   */
  public USBMonitor.UsbDeviceInfo getDeviceInfo(final UsbDevice device) {
   ...
   * @return
   */
  public static USBMonitor.UsbDeviceInfo getDeviceInfo(final Context context, final UsbDevice device) {
    return updateDeviceInfo((UsbManager)context.getSystemService(Context.USB_SERVICE), device, new USBMonitor.UsbDeviceInfo());
  }
   ...
   * @return
   */
  public static USBMonitor.UsbDeviceInfo updateDeviceInfo(final UsbManager manager, final UsbDevice device, final USBMonitor.UsbDeviceInfo _info) {
    final USBMonitor.UsbDeviceInfo info = _info != null ? _info : new USBMonitor.UsbDeviceInfo();
    info.clear();
...
//			}
     ...
     * @throws IllegalStateException
     */
    private UsbControlBlock(final USBMonitor.UsbControlBlock src) throws IllegalStateException {
    ...
    public USBMonitor.UsbControlBlock clone() throws CloneNotSupportedException {
      ...
      try {
        ctrlblock = new USBMonitor.UsbControlBlock(this);
      ...
      if (mConnection != null) {
        ...
        if (monitor != null) {
          if (monitor.mOnDeviceConnectListener != null) {
            monitor.mOnDeviceConnectListener.onDisconnect(mWeakDevice.get(), USBMonitor.UsbControlBlock.this);
          }
